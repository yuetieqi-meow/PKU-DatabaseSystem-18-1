...
while (list($isbn, $author, $name, $press, $cin, $ein, $type, $img) = $result->fetch_row()) {
    ...
    while (list($owner, $number, $price) = $warehouse_result->fetch_row()) {
        ...
        echo '	<div class="sell_information">
            <form action="book_change.php" method="post">
			&nbsp; <div>卖家：' . $owner_name . '</div>
            &nbsp;<div>&nbsp;库存：<input name="number" class="box" type="number" required="required" value="' . $number . '"  "/></div>
            &nbsp;&nbsp;<div>价格：<input name="price"  class="box" type="number" required="required" value="' . $price . '"  "/></div>
            <input name="isbn" value="' . $isbn . '" style="display:none; "/>
            <input name="cID" value="' . $owner . '" style="display:none; "/>
            <div><button class="button" type="submit" id="button" style="z-index: 3;display: block align-items: center;">修改</button></div>
            </form>
                
            <form action="book_delete.php" method="post">
            <input name="isbn" value="' . $isbn . '" style="display:none; "/>
            <input name="cID" value="' . $owner . '" style="display:none; "/>
           <div> <button class="button" type="submit" id="button" style="z-index: 3;display: block align-items: center;">删除</button></div>
            </form>
